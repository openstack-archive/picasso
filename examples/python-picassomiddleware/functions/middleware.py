...
class FunctionsWebhookMiddleware(object):
    ...
    def __init__(self, app, conf):
        self.app = app
        self.logger = get_logger(conf, log_route='picasso_functions')

    def __call__(self, env, start_response):
        req = Request(env)
        resp = req.get_response(self.app)
        self.logger.info("Picasso: available headers: {}".format(str(dict(req.headers))))
        try:
            if "X-Function-URL" in req.headers:
                version, account, container, obj = split_path(req.path_info, 4, 4, True)
                self.logger.info("Picasso version {}, account {}, container {}, object {}"
                ...
                if obj and is_success(resp.status_int) and req.method == 'PUT':
                    ...
                    self.logger.info("Picasso data to send to a function {}"
                                     .format(str(data)))
                    ...
                    self.logger.info("Picasso data to send as bytes {}"
                                     .format(data_as_bytes))
                    with Timeout(60):
                        try:
                            result = urllib2.urlopen(webhook_req).read()
                            self.logger.info(
                                "Picasso function worked fine. Result {}"
                                .format(str(result)))
                        except (Exception, Timeout) as ex:
                            self.logger.error(
                                'Picasso failed POST to webhook {}, '
                                'error {}'.format(webhook, str(ex)))
            else:
                self.logger.info("Picasso skipping functions middleware "
